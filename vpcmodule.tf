...
################################################################################
# Managed Prefix List for TGW Routes
################################################################################

resource "aws_ec2_managed_prefix_list" "tgw_routes" {
  name           = "Transit Gateway Routes"
  address_family = "IPv4"
  max_entries    = 50
  ...
  dynamic "entry" {
    ...
    content {
      cidr        = entry.value
      description = "TGW Route ${entry.value}"
    }
  }
}
...
resource "aws_internet_gateway" "this" {
  count = var.create_vpc && var.create_igw && (length(var.public_subnets) > 0 || length(var.firewall_subnets) > 0) ? 1 : 0
...
resource "aws_route" "firewall_internet_gateway_ipv6" {
  count = var.create_vpc && var.create_igw && var.enable_ipv6 && length(var.firewall_subnets) > 0 ? 1 : 0
...
################################################################################
# Transit Gateway Private Routes
################################################################################

resource "aws_route" "private-transit-gateway" {
  count = var.transit_gateway_id != null ? length(aws_route_table.private.*) : 0

  route_table_id             = element(aws_route_table.private.*.id, count.index)
  destination_prefix_list_id = aws_ec2_managed_prefix_list.tgw_routes.id
  transit_gateway_id         = var.transit_gateway_id

  depends_on = [
    aws_ec2_transit_gateway_vpc_attachment.this,
    aws_ec2_managed_prefix_list.tgw_routes
  ]
}

################################################################################
# Transit Gateway Intra Routes
################################################################################

resource "aws_route" "intra-transit-gateway" {
  count = var.transit_gateway_id != null ? length(aws_route_table.intra.*) : 0
